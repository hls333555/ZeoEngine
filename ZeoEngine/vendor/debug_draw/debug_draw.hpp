...
// ========================================================
    ...
    static inline void vecOrthogonalBasis(ddVec3_Out right, ddVec3_Out up, ddVec3_In v)
    {
        // Produces two orthogonal vectors for normalized vector v.
        // Fix: Calculate using cross product
        ddVec3 worldUp;
        vecSet(worldUp, 0.0f, 1.0f, 0.0f);
        if (v[X] == worldUp[X] && v[Y] == worldUp[Y] && v[Z] == worldUp[Z])
        {
        ...
        right[X] = v[Y] * worldUp[Z] - worldUp[Y] * v[Z];
        right[Y] = v[Z] * worldUp[X] - worldUp[Z] * v[X];
        right[Z] = v[X] * worldUp[Y] - worldUp[X] * v[Y];
        vecNormalize(right, right);

        up[X] = right[Y] * v[Z] - v[Y] * right[Z];
        up[Y] = right[Z] * v[X] - v[Z] * right[X];
        up[Z] = right[X] * v[Y] - v[X] * right[Y];
        vecNormalize(up, up);
    }
...
#undef DD_PLANE_V
        ...
        const float radius, const int durationMillis, const bool depthEnabled)
    {
        ...
        vecSet(radiusVec, 0.0f, radius, 0.0f);
        ...
        {
            const float s = floatSin(degreesToRadians(i + 90));
            const float c = floatCos(degreesToRadians(i + 90));

            lastPoint[X] = center[X] + radius * c;
            lastPoint[Y] = center[Y] + radius * s;
            lastPoint[Z] = center[Z];

            for (int n = 0, j = stepSize; j <= 360; j += stepSize, ++n)
            {
                temp[X] = center[X] + floatCos(degreesToRadians(j)) * radius * c;
                temp[Y] = lastPoint[Y];
                temp[Z] = center[Z] + floatSin(degreesToRadians(j)) * radius * c;
