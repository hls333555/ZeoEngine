		...
		bool Draw(AssetHandle& outHandle, float rightPadding, AssetRightClickFunc rightClickFunc, bool bAllowClear = true)
		{
			...
			ImGui::PushID(TypeID);
			{
				...
				ImGui::BeginGroup();
				{
					...
					{
						if (ImGui::TransparentButton(ICON_FA_SIGN_IN_ALT))
						{
							const auto* contentBrowser = g_Editor->GetPanel<ContentBrowserPanel>(CONTENT_BROWSER);
							const auto& selectedPath = contentBrowser->GetSelectedPath();
							if (const AssetHandle selectedAsset = AssetRegistry::Get().GetAssetHandleFromPath(selectedPath))
							{
								bIsValueChanged = outHandle != selectedAsset;
								outHandle = selectedAsset;
							}
						}

						if (ImGui::IsItemHovered())
						{
							ImGui::SetTooltipWithPadding("Select asset from ContentBrowser");
						}
					}
					...
					{
						if (ImGui::TransparentButton(ICON_FA_SEARCH))
						{
							auto* contentBrowser = g_Editor->GetPanel<ContentBrowserPanel>(CONTENT_BROWSER);
							if (metadata)
							{
								contentBrowser->SetSelectedPath(metadata->Path);
							}
						}

						if (ImGui::IsItemHovered())
						{
							ImGui::SetTooltipWithPadding("Focus asset in ContentBrowser");
						}
					}
				}
				ImGui::EndGroup();
